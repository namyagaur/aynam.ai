...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: `Create 8 original, specific ${mode} speaking-practice prompts for a 2-5 minute session. Avoid generic prompts such as “Tell me about yourself”. Return exactly this JSON object and no other text: {"topics":["..."]}.`,
      config: { responseMimeType: "application/json" },
    });
    const parsed = JSON.parse(response.text ?? "{}") as { topics?: unknown };
    const topics = Array.isArray(parsed.topics) ? parsed.topics.filter((item): item is string => typeof item === "string").slice(0, 8) : [];
    if (topics.length < 5) throw new Error("Too few topics returned.");
