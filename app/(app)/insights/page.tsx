  ...
  return (
    <main className="min-h-full bg-[#FFFDF7] px-4 py-4 text-[#24232B] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1180px]">
        ...
        {activeTab === "progress" ? <ProgressView trend={progressPoints} profile={progressSkills} sessions={visible} recentSessions={sessions} /> : null}
    ...
    <section className="mt-4 grid gap-3 md:grid-cols-12">
      <div className="rounded-[18px] border border-[rgba(36,35,43,0.08)] bg-white p-4 shadow-[0_4px_14px_rgba(42,35,55,0.04)] md:col-span-8 md:grid md:grid-cols-[0.78fr_1.22fr] md:gap-4">
        <div className="border-b border-[rgba(36,35,43,0.08)] pb-4 md:border-b-0 md:border-r md:pb-0 md:pr-4">
          ...
          <div className="mt-2 flex items-end gap-2"><span className="text-[48px] font-semibold leading-none tracking-[-0.08em] text-[#24232b]">{score ?? "—"}</span>{score !== null ? <span className="pb-1.5 text-[14px] text-[#7b8192]">/ 100</span> : null}</div>
...
function PatternsView({ pacePoints, paceTrend, averagePace, fillerTotal, fillerWords, fillerTrend, sentenceAverage, utilizationRatio, totalSpeakingSeconds, totalElapsedSeconds, topTopics, visible }: {
  ...
  const fillerText = fillerTrend === "down" ? "You’re using fewer filler words compared to earlier sessions." : fillerTotal > 0 ? "Aynam is still learning how your filler words are changing." : "No filler words have been detected in this range.";
...
function ProgressView({ trend, profile, sessions, recentSessions }: { trend: Array<{ label: string; score: number; session: number }>; profile: Array<{ label: string; first: number | null; latest: number | null; change: number | null }>; sessions: Session[]; recentSessions: Session[] }) {
  ...
  const headline = overallChange === null ? "Your baseline is ready." : overallChange > 0 ? "You’re making progress." : overallChange < 0 ? "Your recent sessions look a little different." : "You’re holding steady.";
...
function SkillProgressRow({ item }: { item: { label: string; first: number | null; latest: number | null; change: number | null } }) {
  if (item.change === null || item.first === null || item.latest === null) return <div className="flex items-center justify-between border-b border-[#ecebf3] py-2 text-[12px]"><span className="text-[#24232b]">{item.label}</span><span className="text-[#9797a6]">—</span></div>;
